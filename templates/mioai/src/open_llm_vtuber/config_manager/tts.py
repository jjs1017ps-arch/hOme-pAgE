...
class MinimaxTTSConfig(I18nMixin):
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        ...
        "pronunciation_dict": Description(
            en="Custom pronunciation dictionary (string)", zh="自定义发음字典（字符串）"
        ),
    }
...
class ElevenLabsTTSConfig(I18nMixin):
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        ...
        "voice_id": Description(
            en="Voice ID to use for ElevenLabs TTS", zh="ElevenLabs TTS 使用의 语音 ID"
        ),
...
class TypecastTTSConfig(I18nMixin):
    """Configuration for Typecast TTS."""

    api_key: str = Field(..., alias="api_key")
    actor_id: str = Field(..., alias="actor_id")
    speech_tempo: float = Field(1.0, alias="speech_tempo")
    audio_format: str = Field("mp3", alias="audio_format")

    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        "api_key": Description(
            en="API key for Typecast TTS service", zh="Typecast TTS 服务的 API 密钥"
        ),
        "actor_id": Description(
            en="Actor ID to use for Typecast TTS", zh="Typecast TTS 使用的演员 ID"
        ),
        "speech_tempo": Description(en="Speaking tempo", zh="语速"),
        "audio_format": Description(en="Audio format (mp3 or wav)", zh="音频格式"),
    }
...
class TTSConfig(I18nMixin):
    ...
    typecast_tts: Optional[TypecastTTSConfig] = Field(None, alias="typecast_tts")
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        ...
        "typecast_tts": Description(
            en="Configuration for Typecast TTS", zh="Typecast TTS 配置"
        ),
    }
    ...
    def check_tts_config(cls, values: "TTSConfig", info: ValidationInfo):
        ...
        elif tts_model == "typecast_tts" and values.typecast_tts is not None:
            values.typecast_tts.model_validate(values.typecast_tts.model_dump())
